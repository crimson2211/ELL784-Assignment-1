...
class GMMBackgroundSubtractor:
    ...
    def init_video_writers(self, frame_shape):
        ...
        self.original_writer = cv2.VideoWriter(
            os.path.join(self.output_path, 'original_sum_output.avi'), fourcc, 30.0,
            (frame_shape[1], frame_shape[0]), True
        )
        self.background_writer = cv2.VideoWriter(
            os.path.join(self.output_path, 'background_sum_output.avi'), fourcc, 30.0,
            (self.width, self.height), False
        )
        self.foreground_writer = cv2.VideoWriter(
            os.path.join(self.output_path, 'foreground_sum_output.avi'), fourcc, 30.0,
            (self.width, self.height), False
        )
...
if __name__ == '__main__':
    video = cv2.VideoCapture("/content/summary.avi")
